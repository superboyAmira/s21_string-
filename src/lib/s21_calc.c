...
s21_size_t s21_strspn(const char *str1, const char *str2) {
  ...
  if ((str1 != s21_NULL) && (str2 != s21_NULL)) {
    for (s21_size_t i = 0; i < s21_strlen(str1); i++) {
      ...
      if (res == 0)
        break;  // если res == 0, значит, совпадения не нашлись и можно
                // проверять дальше
    }
  }
  return len;
...
s21_size_t s21_strcspn(const char *str1, const char *str2) {  // начальный
  // сегмент!
  s21_size_t len = 0;
  if ((str1 != s21_NULL) && (str2 != s21_NULL)) {
    while (*str1 != '\0') {
      if (s21_strchr(str2, *str1) != s21_NULL) {
        break;  // ищется первое
                // вхождение каждого из символов str1 внутри str2
